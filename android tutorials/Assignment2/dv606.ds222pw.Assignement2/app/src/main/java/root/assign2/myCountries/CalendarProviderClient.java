...
public interface CalendarProviderClient extends LoaderManager.LoaderCallbacks<Cursor> {
     ...
     *
     * This table holds the calendar-specific information.
     * Each row in this table contains the details for a single calendar,
     * such as the name, color, sync information, and so on.
     */
    // this selects all  calendar tables
    // you can also access one row in the table: Uri singleUri = ContentUris.withAppendedId(UserDictionary.Words.CONTENT_URI,4);
    Uri CALENDARS_LIST_URI = CalendarContract.Calendars.CONTENT_URI;

    /**
     * Selection used when querying for MyCountries calendar
     * Name is MyCountries, Account Type is ? local ?
     */
    // in SQL: "WHERE col = value"
     ...
     */
    // replaces the "?" sign in the selection clause, it is the value in SQL: "WHERE col = value"
     ...
     */
    // which columns/attributes of the objects I want to access
     ...
     *
     * This table holds the event-specific information.
     * Each row in this table has the information for a single event—for example,
     * event title, location, start time, end time, and so on.
     */
